...
1. **아무도 안 건드린 조서** — 플레이가 먼저 끝난 순서대로, 한 사람에 하나
2. 그런 게 없는데 **아직 판결 안 난 조서를 누가 읽고 있으면 그걸 같이** — 동시에 들어온
   사람끼리 나눠 읽는다. 여럿이면 덜 붙들린 쪽부터
3. 그것도 없으면 **덜 읽힌 조서부터 다시** — 몇 번을 읽히든 상한은 없다
4. 줄 게 정말 하나도 없을 때만 지어낸 조서

```
대기열: 서혁인 하나

A·B 가 동시에 들어옴   → 둘 다 서혁인 (2순위)
A 가 먼저 끝냄        → 대기열 1: A 의 진술
B 가 나중에 끝냄      → 대기열 2: B 의 진술
C·D 가 동시에 들어옴   → C 는 A 의 진술, D 는 B 의 진술 (1순위)
```

대기열 순서는 **진술을 제출한 시각**, 곧 게임을 끝낸 순서다. 판결은 게임 초반(제2장)에
내리고 진술은 맨 끝에 내므로, 보통은 먼저 판결한 사람이 먼저 끝낸다.

거르는 것은 셋이다. **자기 진술**, **자기가 이미 판결한 조서**, 손으로 **빼둔 조서**.
판결까지 안 가고 나간 사람은 30분(`CLAIM_TTL_MS`) 뒤 자리를 놓는다.

**통지는 한 조서에 두 통까지**(`MAX_MAILS`)다. 판결이 서로 갈려도 맞추지 않는다 —
판결 두 개가 나란히 남고, 조서 번호(`/?t=…`)로 들어가면 순서대로 다 보인다.

## 대기열 손으로 정리하기

`tools/admin.js`. 쓰는 명령은 먼저 무엇을 할지만 보여주고, `--yes` 를 붙여야 실제로 한다.

```bash
export DATABASE_URL='postgresql://...'

node tools/admin.js list                       # 대기열 · 판결 난 조서 · 빼둔 조서
node tools/admin.js show 서혁인                # 진술서 — 게임에서 보는 그대로
node tools/admin.js judge 서혁인 유죄 "판결문" "탐정 이름" --yes   # 플레이 없이 판결 (통지도 나감)
node tools/admin.js forget-email 하요하요 --yes   # 못 쓰는 주소 지우기
node tools/admin.js park 정한결 --yes           # 대기열에서 빼두기
node tools/admin.js unpark 정한결 --yes         # 되돌리기
```

이름이 겹치면 `list` 에 찍힌 `#번호` 앞자리로 가리킨다(`show '#5b87'`).

판결은 **프로덕션 서버를 거쳐** 나간다. 로컬에는 Brevo 키가 없어서다. 서버는 조서를 받아 간
사람만 판결하게 막혀 있으므로, 관리용 손자국을 DB 에 먼저 남기고 그 이름으로 판결한다.
게임에서 판결한 것과 똑같이 기록되고, 통지 결과(`나감 · brevo · <번호>`)도 남는다.

Brevo 로 나간 메일은 **Gmail 보낸편지함에 남지 않는다.** 나갔는지는 `list` 의 `[통지 나감]`,
또는 Brevo 대시보드 → Transactional → Logs 에서 본다.
...
tools/
  ...
  admin.js    대기열을 손으로 정리한다 — 플레이 없이 판결, 주소 지우기, 빼두기
```
...
| | |
|---|---|
| `POST /api/case` | 대기열 맨 앞의 진술을 꺼내 자리 하나를 잡는다 (두 자리, 30분 뒤 자동 해제) |
| `POST /api/verdict` | 판결을 기록하고 앞사람에게 메일을 보낸다. 판결은 몇 번이든, 메일은 두 통까지 |
