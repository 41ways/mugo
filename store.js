...
class FileStore {
  ...
  // 이 브라우저(플레이어 번호)로 낸 가장 최근 진술
  async latestOf(player) {
    return this.rows.filter((r) => r.player === player)
...
class PgStore {
  ...
  async latestOf(player) {
    const { rows } = await this.pool.query(
      'SELECT * FROM statements WHERE player = $1 ORDER BY created_at DESC LIMIT 1', [player]);
    return rows[0] || null;
  }
