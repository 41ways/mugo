...
(async () => {
  ...
  const open = rows.filter((r) => cnt(r) === 0 && Number(r.claimed_at || 0) < PARK)
  ...
  console.log(`\n진술 ${rows.length}건 — 아직 아무도 안 읽은 것 ${open.length}, 판결이 하나라도 난 것 ${done.length}` +
    (held.length ? `, 손으로 빼둔 것 ${held.length}` : '') + '\n');

  if (open.length) {
    console.log('── 대기열 (아직 아무도 안 읽은 것) ──');
    open.forEach((r, i) => {
      ...
      console.log(`${String(i + 1).padStart(2)}. ${r.name}  ${span(now - Number(r.created_at))}째` +
        `  [읽힘 ${read}회${holds.length ? ', 지금 ' + holds.length + '명이 보는 중' : ''}]` +
